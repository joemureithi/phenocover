...
class WeatherDataIntegrator:
    ...
    def get_real_weather_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        if not self.location_data:
            raise ValueError("Location data not loaded. Call load_location_from_geojson() first.")
    ...
    def calculate_growing_degree_days(self, df: pd.DataFrame, base_temp: float = 0.0) -> pd.DataFrame:
        ...
        df['gdd_daily'] = np.maximum(0, (df['temperature_max'] + df['temperature_min']) / 2 - base_temp)
    ...
    def calculate_weather_stress_indices(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['drought_stress'] = df.groupby((df['precip_binary'] != df['precip_binary'].shift()).cumsum())['precip_binary'].cumsum()
        df['drought_stress'] = np.where(df['precip_binary'] == 0, df['drought_stress'], 0)
        df['optimal_conditions'] = ((df['temperature_min'] >= 10) & (df['temperature_max'] <= 25) & (df['precipitation'] > 0)).astype(int)
        return df
    ...
    def generate_synthetic_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        for date in date_range:
            ...
            seasonal_temp = 8.5 + 16.0 * np.sin(2 * np.pi * (day_of_year - 80) / 365)
            ...
            seasonal_precip = 2.1 + 2.5 * np.sin(2 * np.pi * (day_of_year - 200) / 365)
            precipitation = max(0, np.random.exponential(max(0.1, seasonal_precip)))
...
class WheatPhenologyAnalyzer:
    ...
    def load_location_and_weather(self, geojson_file: str):
        ...
        try:
            location_data = self.weather_integrator.load_location_from_geojson(geojson_file)
            ...
            self.weather_data = self.weather_integrator.get_real_weather_data(start_date, end_date)
    ...
    def interpolate_ndvi(self, method: str = 'balanced') -> pd.DataFrame:
        ...
        confidence_intervals = self._calculate_simple_confidence_intervals_fixed(y_interp)
        ...
        if self.has_weather_data:
            ...
            for col in weather_columns:
                if col in self.daily_ndvi.columns:
                    self.daily_ndvi[col] = self.daily_ndvi[col].interpolate(method='linear')
    ...
    def _calculate_simple_confidence_intervals(self, x_obs: np.ndarray, y_obs: np.ndarray, 
        ...
        base_uncertainty = max(0.02, uncertainty_factor * ndvi_range)  # Minimum 0.02 NDVI uncertainty
    ...
    def _calculate_simple_confidence_intervals_fixed(self, y_interp: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        ndvi_range = np.max(self.ndvi_data['NDVI'].values) - np.min(self.ndvi_data['NDVI'].values)
        ...
        upper_ci[small_mask] = np.maximum(upper_ci[small_mask], y_interp[small_mask] + 0.02)
    ...
    def create_visualization(self, save_path: str = 'wheat_phenology_analysis.png') -> plt.Figure:
        ...
        for stage, date in growth_stage_dates.items():
            ax1.axvline(x=date, color=stage_colors[stage], linestyle='--', alpha=0.8, linewidth=2)
        ...
        # Add vertical markers for growth stage transitions
        for stage, date in growth_stage_dates.items():
            ax2.axvline(x=date, color=stage_colors[stage], linestyle='--', alpha=0.8, linewidth=2)
            ax2.text(date, ax2.get_ylim()[1]*0.95, stage, rotation=90, 
                    verticalalignment='top', fontsize=8, color=stage_colors[stage], fontweight='bold')
        
        ax2.set_xlabel('Date')
        ax2.set_ylabel('Ground Cover Percentage (%)')
        ax2.set_title('Wheat Ground Cover Percentage Time Series', fontsize=14, fontweight='bold')
        ...
        if has_weather:
            ...
            # Add vertical markers for growth stage transitions
            for stage, date in growth_stage_dates.items():
                ax3.axvline(x=date, color=stage_colors[stage], linestyle='--', alpha=0.8, linewidth=2)
            ...
            ax3.set_title('Weather Conditions During Growing Season', fontsize=14, fontweight='bold')
        ...
        if has_weather and 'gdd_cumulative' in self.daily_ndvi.columns:
            ...
            # Add vertical markers for growth stage transitions
            for stage, date in growth_stage_dates.items():
                ax4.axvline(x=date, color=stage_colors[stage], linestyle='--', alpha=0.8, linewidth=2)
                ax4.text(date, ax4.get_ylim()[1]*0.95, stage, rotation=90, 
                        verticalalignment='top', fontsize=8, color=stage_colors[stage], fontweight='bold')
            
            ax4.set_xlabel('Date')
            ax4.set_ylabel('Growing Degree Days (GDD)')
            ax4.set_title('Growing Degree Days and Stage Thresholds', fontsize=14, fontweight='bold')
    ...
    def save_results(self, output_file: str = 'wheat_phenology_results.csv') -> pd.DataFrame:
        ...
        if self.has_weather_data:
            weather_columns = ['temperature_2m', 'temperature_min', 'temperature_max', 
                              'precipitation', 'humidity', 'pressure', 'wind_speed', 
                              'cloud_cover', 'gdd_daily', 'gdd_cumulative']
            weather_columns = [col for col in weather_columns if col in result_df.columns]
            base_columns = base_columns + weather_columns
        
        # Select available columns
        available_columns = [col for col in base_columns if col in result_df.columns]
    ...
    def generate_summary_report(self) -> None:
        ...
        if self.has_weather_data:
            print(f"\nWEATHER CONDITIONS:")
            print("-" * 40)
            print(f"Average Temperature: {self.daily_ndvi['temperature_2m'].mean():.1f}°C")
            print(f"Temperature Range: {self.daily_ndvi['temperature_min'].min():.1f}°C to {self.daily_ndvi['temperature_max'].max():.1f}°C")
            print(f"Total Precipitation: {self.daily_ndvi['precipitation'].sum():.1f} mm")
            print(f"Average Humidity: {self.daily_ndvi['humidity'].mean():.1f}%")
            
            if 'gdd_cumulative' in self.daily_ndvi.columns:
                print(f"Total Growing Degree Days: {self.daily_ndvi['gdd_cumulative'].iloc[-1]:.0f}")
        ...
        for stage in self.growth_stages.keys():
            ...
            if not stage_data.empty:
                ...
                print(f"{stage:15s}: Mean NDVI = {mean_ndvi:.3f}, FVC = {mean_fvc:.3f}, Ground Cover = {mean_ground_cover:.1f}%")
                ...
                if self.has_weather_data and 'temperature_2m' in stage_data.columns:
                    ...
                    print(f"{'':15s}  Mean Temperature = {mean_temp:.1f}°C, Total Precipitation = {total_precip:.1f} mm")
                print()
